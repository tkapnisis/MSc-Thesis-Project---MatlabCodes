...
for i=1:length(lambda)
        ...
        wave_param.beta = beta(1);       % Encounter angle [rad]
        [dw,wave_param] = Wave_Model(t,wave_param,foil_loc,param);
        mu_syn_res.y_dist_lambda_0(:,:,i,:) = lsim_uss(mu_syn_data.Sp*Gd_p,dw,t,samples);
        hinf_res.y_dist_lambda_0(:,:,i,:) = lsim_uss(hinf_data.Sp*Gd_p,dw,t,samples);
        i
    for j=1:samples
...
for i=1:length(lambda)
        ...
        wave_param.beta = beta(2);       % Encounter angle [rad]
        [dw,wave_param] = Wave_Model(t,wave_param,foil_loc,param);
        mu_syn_res.y_dist_lambda_pi(:,:,i,:) = lsim_uss(mu_syn_data.Sp*Gd_p,dw,t,samples);
        hinf_res.y_dist_lambda_pi(:,:,i,:) = lsim_uss(hinf_data.Sp*Gd_p,dw,t,samples);
        i
    for j=1:samples         
