...
load('LTI_Perturbed_Plant.mat','Gp','Gd_p_app','Gsm_p')
...
% load('Simulations_Results_RMS_mult.mat')
% Equilibrium input
...
for i=1:length(omega)
        ...
        [dw,wave_param] = Wave_Model(t,wave_param,foil_loc,param);
        mu_syn_res.y_dist_omega_0(:,:,i,:) = lsim_uss(mu_syn_data.Sp_app*Gd_p_app,dw,t,samples);
        hinf_res.y_dist_omega_0(:,:,i,:) = lsim_uss(hinf_data.Sp_app*Gd_p_app,dw,t,samples);
        i
    for j=1:samples         
...
for i=1:length(lambda)
        ...
        wave_param.beta = beta(1);       % Encounter angle [rad]
        [dw,wave_param] = Wave_Model(t,wave_param,foil_loc,param);
        mu_syn_res.y_dist_lambda_0(:,:,i,:) = lsim_uss(mu_syn_data.Sp_app*Gd_p_app,dw,t,samples);
        hinf_res.y_dist_lambda_0(:,:,i,:) = lsim_uss(hinf_data.Sp_app*Gd_p_app,dw,t,samples);
        i
    for j=1:samples
        mu_syn_res.RMS_lambda_0(:,i,j) = rms(mu_syn_res.y_dist_lambda_0(:,:,i,j));
        hinf_res.RMS_lambda_0(:,i,j) = rms(hinf_res.y_dist_lambda_0(:,:,i,j));
    end
end
disp('lambda')
for i=1:length(zeta_0)
        ...
        wave_param.beta = beta(1);       % Encounter angle [rad]
        [dw,wave_param] = Wave_Model(t,wave_param,foil_loc,param);
        mu_syn_res.y_dist_zeta_0_0(:,:,i,:) = lsim_uss(mu_syn_data.Sp_app*Gd_p_app,dw,t,samples);
        hinf_res.y_dist_zeta_0_0(:,:,i,:) = lsim_uss(hinf_data.Sp_app*Gd_p_app,dw,t,samples);
        i
    for j=1:samples
...
for i=1:length(omega)
        ...
        [dw,wave_param] = Wave_Model(t,wave_param,foil_loc,param);
        mu_syn_res.y_dist_omega_pi(:,:,i,:) = lsim_uss(mu_syn_data.Sp_app*Gd_p_app,dw,t,samples);
        hinf_res.y_dist_omega_pi(:,:,i,:) = lsim_uss(hinf_data.Sp_app*Gd_p_app,dw,t,samples);
        i
    for j=1:samples         
...
for i=1:length(lambda)
        ...
        wave_param.beta = beta(2);       % Encounter angle [rad]
        [dw,wave_param] = Wave_Model(t,wave_param,foil_loc,param);
        mu_syn_res.y_dist_lambda_pi(:,:,i,:) = lsim_uss(mu_syn_data.Sp_app*Gd_p_app,dw,t,samples);
        hinf_res.y_dist_lambda_pi(:,:,i,:) = lsim_uss(hinf_data.Sp_app*Gd_p_app,dw,t,samples);
        i
    for j=1:samples         
        mu_syn_res.RMS_lambda_pi(:,i,j) = rms(mu_syn_res.y_dist_lambda_pi(:,:,i,j));
        hinf_res.RMS_lambda_pi(:,i,j) = rms(hinf_res.y_dist_lambda_pi(:,:,i,j));
    end
end
disp('lambda')
for i=1:length(zeta_0)
        ...
        wave_param.beta = beta(2);       % Encounter angle [rad]
        [dw,wave_param] = Wave_Model(t,wave_param,foil_loc,param);
        mu_syn_res.y_dist_zeta_0_pi(:,:,i,:) = lsim_uss(mu_syn_data.Sp_app*Gd_p_app,dw,t,samples);
        hinf_res.y_dist_zeta_0_pi(:,:,i,:) = lsim_uss(hinf_data.Sp_app*Gd_p_app,dw,t,samples);
        i
    for j=1:samples
...
hold on
for i=1:samples
    plot(lambda,mu_syn_res.RMS_lambda_0(1,:,i),'b--o')
    plot(lambda,hinf_res.RMS_lambda_0(1,:,i),'-.v','Color','#77AC30')
    plot(lambda,mu_syn_res.RMS_lambda_pi(1,:,i),'r--o')
    plot(lambda,hinf_res.RMS_lambda_pi(1,:,i),'-.v','Color','#EDB120')    
end    
...
ax = gca;
ax.FontSize = 10;
xlabel('Wave length (m)','FontSize',12)
...
hold on
for i=1:samples
    plot(lambda,rad2deg(mu_syn_res.RMS_lambda_0(2,:,i)),'b--o')
    plot(lambda,rad2deg(hinf_res.RMS_lambda_0(2,:,i)),'-.v','Color','#77AC30')
    plot(lambda,rad2deg(mu_syn_res.RMS_lambda_pi(2,:,i)),'r--o')
    plot(lambda,rad2deg(hinf_res.RMS_lambda_pi(2,:,i)),'-.v','Color','#EDB120')    
end    
grid minor
ax = gca;
ax.FontSize = 10;
...
hold on
for i=1:samples
    plot(lambda,rad2deg(mu_syn_res.RMS_lambda_0(3,:,i)),'b--o')
    plot(lambda,rad2deg(hinf_res.RMS_lambda_0(3,:,i)),'-.v','Color','#77AC30')
    plot(lambda,rad2deg(mu_syn_res.RMS_lambda_pi(3,:,i)),'r--o')
    plot(lambda,rad2deg(hinf_res.RMS_lambda_pi(3,:,i)),'-.v','Color','#EDB120')    
end    
grid minor
ax = gca;
ax.FontSize = 10;
...
title('Pitch','FontSize',12)
%% Save data
save('Data Files/Simulations_Results_RMS_mult','hinf_res','mu_syn_res','omega',...
     'lambda', 'zeta_0', 'beta')