...
load('Parameters_Nominal.mat','theta_s_f0', 'theta_s_ap0', 'theta_s_as0')
...
param.I_x = ureal('I_x',3.77,'Percentage',[-20 20]); % Moment of inertia around x- axis [kg*m^2]
param.I_xy = ureal('I_xy',-0.32,'Percentage',[-20 20]);  % Product of inertia around xy-axes [kg*m^2]
param.I_y = ureal('I_y',7.60,'Percentage',[-20 20]);    % Moment of inertia around y- axis [kg*m^2]
...
param.l_xj_f = ureal('l_xj_f',0.402,'Percentage',[-20,20]); 
% x distance between the hinge of fore hydrofoil and CG [m]
param.l_xj_ap = ureal('l_xj_a',-0.298,'Percentage',[-20,20]);   
