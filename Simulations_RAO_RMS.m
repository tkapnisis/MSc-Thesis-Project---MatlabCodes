...
lambda = 0.2:0.2:3;   % Wave length [m]
...
for i=1:length(lambda)
    ...
    mu_syn_res.y_dist(:,:,i) = lsim(mu_syn_data.S*Gd,dw,t);
    hinf_res.y_dist(:,:,i) = lsim(hinf_data.S*Gd,dw,t);
    mu_syn_res.RMS(:,i) = rms(mu_syn_res.y_dist(:,:,i));
    hinf_res.RMS(:,i) = rms(hinf_res.y_dist(:,:,i));
end

%%
figure
subplot(3,1,1)
plot(lambda,mu_syn_res.RMS(1,:),'r-o')
hold on
plot(lambda,hinf_res.RMS(1,:),'b-o')
legend('\boldmath{$\mu$} \textbf{-synthesis}', '\boldmath{$h_{\infty}$} \textbf{-synthesis}',...
       'interpreter','latex','FontSize',10)
grid minor
xlabel('Frequency (rad/s)')

subplot(3,1,2)
plot(lambda,mu_syn_res.RMS(2,:),'r-o')
hold on
plot(lambda,hinf_res.RMS(2,:),'b-o')
legend('\boldmath{$\mu$} \textbf{-synthesis}', '\boldmath{$h_{\infty}$} \textbf{-synthesis}',...
       'interpreter','latex','FontSize',10)
grid minor
xlabel('Frequency (rad/s)')
...
subplot(3,1,3)
plot(lambda,mu_syn_res.RMS(3,:),'r-o')
hold on
plot(lambda,hinf_res.RMS(3,:),'b-o')
legend('\boldmath{$\mu$} \textbf{-synthesis}', '\boldmath{$h_{\infty}$} \textbf{-synthesis}',...
       'interpreter','latex','FontSize',10)
grid minor
xlabel('Frequency (rad/s)')
