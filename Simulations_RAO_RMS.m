...
for i=1:length(omega)
    ...
    wave_param.lambda = lambda(1);   % Wave length [m]
    wave_param.zeta_0 = zeta_0(5);   % Wave amplitude [m]
    wave_param.beta = beta(1);       % Encounter angle [rad]

    [dw,wave_param] = Wave_Model(t,wave_param,foil_loc,param);
    mu_syn_res.y_dist_omega_0(:,:,i) = lsim(mu_syn_data.S*Gd,dw,t);
    hinf_res.y_dist_omega_0(:,:,i) = lsim(hinf_data.S*Gd,dw,t);
    mu_syn_res.RMS_omega_0(:,i) = rms(mu_syn_res.y_dist_omega_0(:,:,i));
    hinf_res.RMS_omega_0(:,i) = rms(hinf_res.y_dist_omega_0(:,:,i));
end

for i=1:length(lambda)
    wave_param.omega = omega(5); % Wave frequency [rad/s]
...
figure
subplot(3,1,1)
plot(omega,mu_syn_res.RMS_omega_0(1,:),'r-o')
hold on
plot(omega,hinf_res.RMS_omega_0(1,:),'r-x')
...
subplot(3,1,2)
plot(omega,mu_syn_res.RMS_omega_0(2,:),'r-o')
hold on
plot(omega,hinf_res.RMS_omega_0(2,:),'r-x')
...
subplot(3,1,3)
plot(omega,mu_syn_res.RMS_omega_0(3,:),'r-o')
hold on
plot(omega,hinf_res.RMS_omega_0(3,:),'r-x')
