...
[~,Info] = ucover(Gp_frd,G,ord1,'OutputMult');
W_1 = Info.W1;
%}
%%
order = 3;
...
disp('----- 2nd method -----')
W_I_mat = [  (W_I.w11*Delta_I(1,1))*G(1,1),...
             (W_I.w12*Delta_I(1,2))*G(1,2),...
             (W_I.w13*Delta_I(1,3))*G(1,3);...
             (W_I.w21*Delta_I(2,1))*G(2,1),...
             (W_I.w22*Delta_I(2,2))*G(2,2),...
             (W_I.w23*Delta_I(2,3))*G(2,3);...
             (W_I.w31*Delta_I(3,1))*G(3,1),...
             (W_I.w32*Delta_I(3,2))*G(3,2),...
             (W_I.w33*Delta_I(3,3))*G(3,3)];
...
sys1 = series(G,W_I_mat);
Gp_inp_mult_3 = parallel(sys1,G);
...
%%
bound = 0.3;
Delta_I = ultidyn('di',[3,3],'Bound',bound);
Gp_app = G*(eye(3) + W_I_ss*Delta_I);


%%
% omega = logspace(-2,3,200);
figure
bodeplot(Gp_inp_mult_3,G,bode_opts)
...
sigma_opts.XLim = [1e-3,1e3];
sigma_opts.YLimMode = 'manual';
sigma_opts.YLim = [1e-6,1e8];
% Gp_add,G,
figure
sigma(Gp_inp_mult_3,G,sigma_opts);
legend('\boldmath{$\sigma(Gp_inp_mult_3,0.2)$}','\boldmath{$\sigma(G)$}','interpreter','latex','FontSize',15)
